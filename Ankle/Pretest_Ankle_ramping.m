...
%
% Triggers: 
%     "trigger 1" : the onset of a trial.
%     "trigger 2" : the onset of the ramping.
%     "trigger 3" : the onset of the threshold.
%     "trigger 4" : the offset of the threshold.
%
% Default parameters:   
...
ch.TerminalConfig = "Differential";                                          % 'Terminal  Config (SingleEnded)', if any...
end
...
oldTextSize=Screen('TextSize', theWindow, 60);                              % Costumize the textsize witht the monitor.  
...
Threshold_t=40;                          % #4-#5 Default is 60.
Rest_t=40;
...
%         end
    ...
    if ~DebugMode, triger4_check = GetSecs-startTime, timing_check=[timing_check; triger4_check]; 
             setRTS(t,  true); setDTR(t, true);io64(ioObj,address,1);setRTS(t,  false); setDTR(t, false);   end % trigger 4: the offset of the threshold.   
